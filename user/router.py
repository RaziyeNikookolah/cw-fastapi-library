...
from user.crud import get_users, get_by_username, insert_user, delete_user
...
@router.post("/register", status_code=status.HTTP_201_CREATED, summary="Regiser user")
def create_user(user: UserRequest) -> Any:
    user_dict = get_by_username(user.username)
    if user_dict:
